...
const toF = c => Math.floor((c * 9) / 5 + 32);
...
const weatherFetchError = (state, action) => ({
  ...state,
  error: action.error
});
...
const weatherDataRecevied = (state, action) => {
  ...
  return {
    ...
    latitude,
    longitude,
    temperatureinCelsius: Math.floor(the_temp),
...
const handlers = {
  ...
  [actions.WEATHER_ID_ERROR]: weatherFetchError,
  [actions.WEATHER_DATA_ERROR]: weatherFetchError
};
